...
/// GET `/api/sessions` — prior Claude transcripts for this workspace that the SPA
/// can offer to resume (newest first).
async fn sessions_handler(State(state): State<AppState>) -> Response {
    Json(state.agents.list_sessions_json()).into_response()
}
